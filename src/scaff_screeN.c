...
/*  process contigs one by one   */
    ...
    for(i=0;i<nSeq;i++)
    {
       ...
       for(j=0;j<seq_ed;j++)
       {
          if((seqp->data[j]=='A')||(seqp->data[j]=='C')||(seqp->data[j]=='G')||(seqp->data[j]=='T')||(seqp->data[j]=='a')||(seqp->data[j]=='c')||(seqp->data[j]=='g')||(seqp->data[j]=='t'))
	  {
	    seq_st = j;
	    j = seq_ed;
	  }	  
       }
       for(j=(seq_ed-1);j>0;j--)
       {
          if((seqp->data[j]=='A')||(seqp->data[j]=='C')||(seqp->data[j]=='G')||(seqp->data[j]=='T')||(seqp->data[j]=='a')||(seqp->data[j]=='c')||(seqp->data[j]=='g')||(seqp->data[j]=='t'))
	  {
	    seq_ed = j+1;
       ...
       nline = (seq_ed-seq_st)/60;
       if((seq_ed-seq_st)>=set_len)
       {
         st = seqp->data+seq_st;
         fprintf(namef,">%s\n",seqp->name);
         for(j=0;j<nline;j++)
